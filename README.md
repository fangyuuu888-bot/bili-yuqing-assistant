...
[![Chrome](https://img.shields.io/badge/Chrome-88+-4285F4?logo=googlechrome)](#浏览器兼容性)
[![Edge](https://img.shields.io/badge/Edge-88+-0078D4?logo=microsoftedge)](#浏览器兼容性)
[![Version](https://img.shields.io/badge/version-2.0.0-blue)](#功能概览)
...
- **评论采集**：在 B站视频页面一键采集评论，或通过输入视频链接远程获取
...
- **用户信息采集**：在 B站用户主页一键采集，或通过输入主页链接远程获取
- **活跃趋势**：按月份统计发文量，展示活跃度变化
- **兴趣领域分布**：基于 150+ 关键词分类，识别用户内容领域偏好
...
| 扩展弹窗 | 安装网站 |
|---------|---------|
| ![扩展弹窗](screenshots/popup.png) | ![安装网站](screenshots/install-website.png) |

| 情感分析 + 高频话题 | 发文词云 + 参与话题 |
|-------------------|-------------------|
| ![情感分析](screenshots/video-result.png) | ![词云话题](screenshots/wordcloud-topics.png) |

| 粉丝互动排行 | 意见领袖排行 |
|------------|------------|
| ![粉丝排行](screenshots/fan-ranking.png) | ![KOL排行](screenshots/kol-comments.png) |
...
![安装步骤示意](screenshots/chrome-extensions.png)
...
2. 点击「采集当前页面评论」按钮，自动采集当前视频评论
3. 或在「视频舆情分析」页面输入视频链接，点击「开始分析」
4. 查看情感分析、词云、KOL 排行等分析结果

![视频分析入口](screenshots/popup.png)
...
| 视频列表（备用 1） | `x/space/arc/search` | 旧版端点 |
| 视频列表（备用 2） | `x/polymer/web-dynamic/v1/feed/space` | 动态流 |
...
- **支持链接直接分析**：无需打开视频页 / 用户页，直接粘贴链接即可获取数据并分析
...
| Edge | 88+ |
| Brave | 最新版 |
| 其他 Chromium 内核浏览器 | 支持 Manifest V3 |
...
- ⬇️ **下载安装包**：[GitHub Releases](https://github.com/fangyuuu888-bot/bili-yuqing-assistant/releases/latest)
